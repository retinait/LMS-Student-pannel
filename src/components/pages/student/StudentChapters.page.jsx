...
const StudentSubjects = props => {
	...
	const [orderedChapter, setOrderedChapter] = useState([]);
	...
	const chapterList = allChapters.filter(item => item.subjectId === subjectId);
	const courseWiseChapter = useSelector(state => state.courses.chapterList);
	...
	useEffect(() => {
		if (
			courseWiseChapter &&
			chapterList &&
			courseWiseChapter.length > 0 &&
			chapterList.length > 0
		) {
			const newArr = [];
			courseWiseChapter.forEach(element => {
				const temp = chapterList.find(item => item._id === element._id);
				if (!!temp) {
					newArr.push(temp);
				}
			});
			setOrderedChapter(newArr);
		}
					...
					<Row style={{ marginRight: 0, marginLeft: 0, marginBottom: 20 }}>
						<Col xs={24}>
							<div className='chapter-lecture-tab'>
								<StudentTabMenu selectedKey='chapters' menuData={tabMenuData} />
							</div>
						</Col>
					</Row>
					{/* {groupIds && groupIds.length <= 0 && chapterList?.length <= 0 ? ( */}
					{groupIds && groupIds.length <= 0 && (
						<Empty
							description={`You are not added to a group yet, To view contents
			 you must be added to a student group.
				Please ask ${config.clientName} Administration to add you to a group.`}
					...
					<Row gutter={[16, 16]} style={{ marginRight: 0, marginLeft: 0 }}>
						{groupIds && groupIds.length > 0 && orderedChapter.map((item, index) => (
